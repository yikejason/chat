...
impl AppState {
    pub async fn create_chat(
        ...
        if !input.members.contains(&(user_id as i64)) {
            return Err(AppError::CreateChatError(
                "User must be a member of the chat".to_string(),
            ));
        }
    ...
    pub async fn fetch_chat_all(&self, user_id: u64, ws_id: u64) -> Result<Vec<Chat>, AppError> {
    ...
    pub async fn update_chat(&self, id: u64, input: CreateChat) -> Result<Chat, AppError> {
        let chat = sqlx::query_as(
            r#"
            UPDATE chats
