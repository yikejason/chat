...
use crate::{AppError, ChatUser, User};
...
impl User {
    ...
    pub async fn create(input: &CreateUser, pool: &PgPool) -> Result<Self, AppError> {
        ...
        let user: User = sqlx::query_as("INSERT INTO users (ws_id, email, fullname, password_hash) VALUES ($1, $2, $3, $4) RETURNING id, ws_id, fullname, email, created_at")
            .bind(ws.id)
            .bind(&input.email)
            .bind(&input.fullname)
            .bind(password_hash)
    ...
    pub async fn verify(input: &SigninUser, pool: &PgPool) -> Result<Option<Self>, AppError> {
        ...
        match user {
            Some(mut user) => {
                // Take the password hash out of the user struct 从 user 中拿走密码哈希  mem::take
...
impl ChatUser {
    pub async fn fetch_by_ids(ids: &[i64], pool: &PgPool) -> Result<Vec<Self>, AppError> {
        // where id = ANY($1)  $1 is an array of ids  id is in the array
...
fn verify_password(password: &str, password_hash: &str) -> Result<bool, AppError> {
    ...
    // Parse password hash string to PasswordHash struct
...
mod tests {
    ...
    fn hash_password_and_verify_should_work() -> Result<()> {
        ...
        println!("Password hash: {}", password_hash.len());
    ...
    async fn create_duplicate_user_should_fail() -> Result<()> {
        ...
        let input = CreateUser::new("none", "Yu Tian", "yut@acme.org", "hunter42");
